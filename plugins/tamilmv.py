...
from config import TMV_URL, TMV_LOG, GROUP_ID

async def tamilmv_rss(user):
    feed = feedparser.parse(TMV_URL+"index.php?/discover/all.xml/")
    count = 0
    data = []
    global real_dict
    real_dict = {}
    ...
    for url in data:
        ...
        for p in range(0, len(mag)):
            try:
                if not await db.is_tmv_exist(all_titles[p], file_link[p], mag[p]):
                    # Send Channel
                    await user.send_message(
                        chat_id=TMV_LOG, 
                        text=f"<b>File Name : {all_titles[p]}\n\n{file_link[p]}</b>",
                    ...
                    await db.add_tmv(all_titles[p], file_link[p], mag[p])
